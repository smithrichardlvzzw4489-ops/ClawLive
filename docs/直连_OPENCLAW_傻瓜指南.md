...
> **注意**：第 3 步（网关）、第 5 步（穿透）执行后的窗口要**一直开着**，不要关。
...
### 第 2 步：启用 /v1/responses 端点（必做，否则发消息会 404）

**操作**：在**启动网关之前**，先确保 OpenClaw 已启用 HTTP API 端点。

打开配置文件（路径因系统而异）：
- **Windows**：`%USERPROFILE%\.openclaw\openclaw.json`
- **Mac / Linux**：`~/.openclaw/openclaw.json`

若文件不存在，新建该文件。在文件中加入（或合并到已有的 `gateway` 配置里）：
...
保存后关闭。**没有这一步，验证连接会成功但发消息会报 404。**

---

### 第 3 步：启动 OpenClaw 网关
...
→ **这个终端窗口不要关**，保持运行。接下来**再开一个新的终端窗口**做第 4 步。
...
**你会看到**：没有报错即成功。记下你设的 `abc123`（或你换成的密码），第 9 步要用。
...
输入后按 **Enter** 执行，把终端**输出的那串字符**记下来或复制，第 9 步要用。
...
**操作**：**再开一个终端**（第 3 步的网关窗口还在运行），输入或粘贴下面命令，按 **Enter** 执行。
...
**立刻做**：复制 `https://` 开头、到 `.ngrok-free.app` 结尾的那一串，后面第 8 步要粘贴到 ClawLive。
...
### 第 8 步：填写 Gateway URL

**操作**：在 **Gateway URL** 输入框里，粘贴第 5 步复制的地址。
...
**操作**：在 **Token** 输入框里，粘贴第 4 步设置或读取的 token（例如 `abc123`）。
...
- **成功**：出现「连接成功」→ 继续第 11 步
- **失败**：检查第 3 步、第 5 步的终端窗口是否还在运行；Gateway URL、Token 是否填对
...
| 序号 | 检查项 |
|------|--------|
| ① | 第 2 步已在配置中启用 `responses.enabled: true` |
| ② | 第 3 步的 `openclaw gateway start` 终端**还在运行** |
| ③ | 第 5 步的 ngrok（或 localtunnel）终端**还在运行** |
| ④ | Gateway URL 填的是 ngrok/localtunnel 输出的**完整 https 地址** |
| ⑤ | Token 与第 4 步设置或读取的**完全一致** |
...
3. 在输出里找 `https://xxx.trycloudflare.com` 地址，第 8 步填到 Gateway URL
...
3. 本机 Tailscale IP 类似 `100.x.x.x`，第 8 步 Gateway URL 填：`http://100.x.x.x:18789`
...
3. 把输出的公网 URL 在第 8 步填到 Gateway URL
...
### 发消息无反应 / 404 Not Found

- **404 或「Gateway 返回 404」**：说明 `/v1/responses` 端点未启用。按第 2 步编辑 `openclaw.json`，加入 `gateway.http.endpoints.responses.enabled: true`，保存后**重启 openclaw gateway**。
- 先点「验证连接」，确认能通过（验证用 `/status`，发消息用 `/v1/responses`，两者不同）
