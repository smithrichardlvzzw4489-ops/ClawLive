...
import { getDemoHtml } from "@/lib/demo-html";
...
/** 单次请求超时（毫秒）。生成长 HTML 时经常超过 2 分钟，默认 4 分钟 */
...
/** 网络类失败时最多尝试次数（含第一次）。ECONNRESET 等需多试几次，默认 5，最大 8 */
function openRouterMaxAttempts(): number {
  const n = Number(process.env.OPENROUTER_MAX_ATTEMPTS);
  const v = Number.isFinite(n) && n >= 1 ? Math.floor(n) : 5;
...
async function callOpenRouter(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      const retry = !last && isTransientOpenRouterError(e);
      ...
      const backoff = Math.min(
        25_000,
        1800 * 2 ** attempt + Math.floor(Math.random() * 1000),
      );
      await sleep(backoff);
    }
  }
