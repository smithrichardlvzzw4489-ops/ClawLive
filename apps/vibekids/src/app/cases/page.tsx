...
import { CREATIVE_KINDS } from "@/lib/creative";
import { SiteNav } from "@/components/SiteNav";
import { SavedWorksGrid } from "@/components/SavedWorksGrid";
import { EXCELLENT_CASES } from "@/data/cases";
import { getWorkSummaries } from "@/lib/works-storage";
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6">
        ...
        <ul className="mb-14 grid gap-5 sm:grid-cols-2">
          {EXCELLENT_CASES.map((c) => (
            ...
            >
              ...
              <Link
                href={`/studio?age=${c.age}&prompt=${encodeURIComponent(c.prompt)}`}
        ...
        <p className="mb-4 text-sm text-slate-600">
          ...
          <Link href="/gallery" className="font-medium text-violet-600 underline">
            作品展示区
          </Link>{" "}
          相同数据源）。
        </p>
