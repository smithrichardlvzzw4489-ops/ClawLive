...
import { bumpWeeklyLike } from "@/lib/client-engagement";
import { CREATIVE_KINDS } from "@/lib/creative";
import { gradientFromWorkId } from "@/lib/work-card-visual";
...
export function WorkCard({ work, variant = "default", animIndex = 0 }: Props) {
  ...
  const like = useCallback(
    async (e: React.MouseEvent) => {
      ...
      setBusy(true);
      try {
        const res = await fetch(`${APP_BASE_PATH}/api/works/${work.id}/like`, {
          method: "POST",
        });
