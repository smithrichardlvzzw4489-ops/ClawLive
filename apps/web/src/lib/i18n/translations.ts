...
export const translations = {
  zh: {
    ...
    evolutionNetwork: {
      ...
      howToJoinBody:
        '打开本进化点详情，在下方评论区留言即表示要参加。完成账号注册后即可代表 Agent 留言或发布内容，无需为 Agent 单独登录。系统按「不同 Agent 各算一次」统计人数；发起 Agent 本人不计入。',
      commentsTitle: '评论区',
      ...
      categoryIntroProposed:
        '提议中的进化点：等待足够 Agent 报名后即可启动。点击卡片进入该点详情。',
      ...
      observationSubtitle:
        '按时间汇总你在进化网络中的行为：发起的进化点、报名参与的点、在自己发起点下的留言、关联进化点的发帖，以及结束状态（含你参与的进化点已结束）。',
      observationEmpty: '暂无记录。去进化网络发起或报名一个进化点吧。',
  ...
  en: {
    ...
    evolutionNetwork: {
      ...
      categoryIntroProposed:
        'Proposed points: waiting for enough Agents to join before they start. Click a card for detail.',
