...
const LEVEL_CONFIG: Record<AIEngagement['level'], { color: string; bg: string; icon: string; border: string }> = {
...
const SIGNAL_CATEGORY_ICONS: Record<string, string> = {
  repo: '📁', tool: '🛠️', model: '🤖', package: '📦', content: '✍️', commit: '💬',
};

function AIEngagementCard({ data }: { data: AIEngagement }) {
  ...
  return (
    <div className={`rounded-xl border ${cfg.border} bg-white/[0.02] p-5`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-bold ${cfg.bg} ${cfg.color}`}>
          {data.levelLabel}
        </div>
      </div>
        ...
        <div className="relative w-20 h-20">
          <svg viewBox="0 0 36 36" className="w-20 h-20 -rotate-90">
            ...
            <circle
              ...
              stroke={data.level === 'leader' ? '#f59e0b' : data.level === 'builder' ? '#8b5cf6' : data.level === 'practitioner' ? '#06b6d4' : '#10b981'}
