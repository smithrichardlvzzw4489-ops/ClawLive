...
8. **硬性时限**：产品要求约 **30 秒内**必须能生成完毕；优先「最小可玩 / 可用原型」，避免超长内联脚本、大段重复结构、海量逐帧数据。`;
...
7. **硬性时限**：约 **30 秒内**必须输出完毕；修改尽量聚焦，避免无关的大段重写。`;
...
/** 产品硬性要求：一次生成/修改从发起到结束（含重试与退避）不得超过此墙钟时间 */
const VIBEKIDS_GENERATE_DEADLINE_MS = 30_000;

/** 单次 HTTP 超时上界，不超过总截止；可用环境变量在 5s～30s 内微调（本地调试） */
function openRouterSingleRequestCapMs(): number {
  const n = Number(process.env.OPENROUTER_TIMEOUT_MS);
  if (Number.isFinite(n) && n >= 5_000 && n <= VIBEKIDS_GENERATE_DEADLINE_MS) {
    return Math.floor(n);
  }
  return VIBEKIDS_GENERATE_DEADLINE_MS;
}

/** 网络类失败时重试次数（含第一次）。总墙钟仍受 VIBEKIDS_GENERATE_DEADLINE_MS 约束 */
...
async function callOpenRouter(
  ...
  const deadline = Date.now() + VIBEKIDS_GENERATE_DEADLINE_MS;
  const cap = openRouterSingleRequestCapMs();
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (remaining < 1_200) {
      ...
      throw new Error("生成超时（30 秒内未完成），请缩短描述后重试");
    }
