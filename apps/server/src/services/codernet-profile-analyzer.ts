/**
 * Codernet 画像分析：基于爬取的 GitHub 数据，调用 LLM 生成技术标签、能力象限、锐评。
 */
...
function buildAnalysisPrompt(data: GitHubCrawlResult): string {
  ...
  return `你是一位资深技术猎头兼毒舌代码评论家。根据以下 GitHub 数据，生成这位开发者的技术画像。
...
请输出**仅一个 JSON 对象**，格式严格如下（不要额外说明文字）：
{
  ...
  "sharpCommentary": "100字以内锐评，用犀利但不失幽默的口吻评价此开发者的技术风格和特点。要有洞察力，像一个比他自己还懂他的人写的。中文。",
  "oneLiner": "一句话标语（10字以内），如'全栈暴走族'、'AI工具狂人'、'基建默默铺路人'"
}

规则：
- techTags 最多 8 个，要具体（如 "React Native"、"Kubernetes"、"LLM应用开发"），不要泛泛的（如 "编程"、"开发"）
- capabilityQuadrant 四个维度各 0-100，根据仓库/语言/topic 客观推断
- sharpCommentary 必须中文，100字以内，要毒舌但准确
- oneLiner 必须中文，10字以内`;
}
...
export async function analyzeGitHubProfile(
  ...
  return {
    techTags: Array.isArray(parsed.techTags)
      ? parsed.techTags.map(String).filter(Boolean).slice(0, 8)
      : [],
    ...
    sharpCommentary: String(parsed.sharpCommentary || '').slice(0, 300),
