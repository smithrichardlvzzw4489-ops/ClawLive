...
/** 与 Next /api/vibekids/generate 一致：单次请求墙钟上限 */
const VIBEKIDS_LLM_DEADLINE_MS = 30_000;
const VIBEKIDS_MAX_OUTPUT_TOKENS = 6144;
...
8. **硬性时限**：产品要求约 **30 秒内**必须生成完毕；优先「最小可玩 / 可用原型」，避免超长内联脚本、大段重复结构、海量逐帧数据。`;
...
export function lobsterRoutes(): Router {
  ...
  router.post('/vibekids-generate', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    const runCompletion = async (
      ...
      const deadline = Date.now() + VIBEKIDS_LLM_DEADLINE_MS;
