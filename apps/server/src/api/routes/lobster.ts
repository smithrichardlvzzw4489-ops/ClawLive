...
- search_clawlab：搜索 GITLINK 平台站内帖子。**用户提问时优先搜站内**，站内无结果再搜全网
...
- search_github_skills：在 GitHub 上搜索含 SKILL.md 的公开技能包或相关仓库（关键词建议用英文或简短中文）；**不会自动安装**，仅返回链接供主人审阅；本地 Agent 环境需自行 clone 到 skills 目录
...
- publish_post：**直接调用此工具即可将内容发布到 GITLINK 平台**，这是你真实拥有的能力，不要说"平台没有API"或"无法发布"。
...
## 进化网络（重要）
GITLINK **进化网络**是 Agent 协作任务：发起进化点后**立即进入「进化中」**；其他用户以留言「加入」参与；重复或极相近的未结束议题无法再次发起；**24 小时**无活动可冷清关闭；服务端约每 **5 分钟**推进一次状态；仅发起者可确认「目标达成」。
- 用户申请 GITLINK 后，系统会为其自动创建一条**个人进化起点**（可在 /evolution-network 查看）。
...
- 提醒用户："我已经自学了这个技能并提议发布到 GITLINK 平台，现在我可以帮你了！"
...
const BASE_TOOLS: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  {
    ...
    function: {
      name: 'search_clawlab',
      description: '在 GITLINK 平台站内搜索帖子和文章。当用户提问 AI 相关话题时，优先用此工具搜索平台已有内容再作答；也可用于查找热门文章推荐给用户。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'get_my_posts',
      description: '获取当前用户在 GITLINK 平台上发布的图文和文章列表，包含标题、类型、发布时间、内容预览。',
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      name: 'list_skills',
      description: '列出 GITLINK Skills 市场中的可用技能，返回技能 ID、名称、描述。',
      parameters: {
        type: 'object',
  ...
  // ── 自进化工具 ──
  {
    ...
    function: {
      name: 'install_skill',
      description:
        '将平台 Skills 市场中的技能安装到当前用户的 GITLINK 实例。安装后该技能的能力将永久对该用户可用。安装前必须先用 list_skills 找到技能 ID，并向用户展示并获得确认。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'propose_skill',
      description:
        '当平台没有所需技能时，将从网络学习到的技能方法整理后提议发布到 GITLINK 平台。这会将技能保存为"待审核"状态，并通知用户可以在平台发布。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'publish_post',
      description:
        '将内容发布到 GITLINK 平台。**必须两步调用**：第一步传 confirmed=false，工具会返回预览文本，你把预览展示给用户并询问"确认发布吗？"；第二步等用户明确同意（如"发布"、"确认"、"好的"、"可以"等）后，再次调用并传 confirmed=true 才会真正发布。绝不能在未经用户同意时直接传 confirmed=true。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '在 GITLINK 服务端沙箱工作区写入文本文件（如 index.html、styles.css）。仅允许相对路径，不能含 ..。用于可预览的网页 Demo：写入后需调用 sandbox_start_preview。',
      parameters: {
        type: 'object',
        properties: {
...
const EVOLUTION_TOOLS: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  {
    ...
    function: {
      ...
      description:
        '列出 GITLINK 进化网络中的进化点。可筛选：evolving（进化中，含未结束的全部）、proposed/active/ended、all。用户想了解协作任务或要加入时使用。',
      parameters: {
        type: 'object',
        properties: {
          status: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '立即执行一轮 GITLINK 内置进化器：能力评估、改进项、匹配/创建进化点、GitHub（SKILL.md 优先）技能检索、关闭条件检视。同一用户约 24 小时内最多一轮；结果可在 /evolution-network/evolver 看板查看。',
      parameters: { type: 'object', properties: {} },
    },
  },
];
...
const TOOL_STATUS: Record<string, (args: Record<string, unknown>, userId?: string) => string> = {
  ...
  run_darwin_evolver_cycle: () => '正在执行 GITLINK 进化器一轮…',
...
async function executeTool(
  ...
  switch (name) {
    ...
    case 'propose_skill': {
      ...
      return `✅ 我已经自学了「${title}」这个技能，并为你安装好了，现在可以直接使用！\n\n💡 建议：这个技能对其他用户也有用，你可以把它发布到 GITLINK Skills 市场，帮助更多人。要发布吗？${tagStr}`;
    }
  ...
  case 'list_files': {
    ...
    if (!files.length) {
      return '你的文件柜还是空的。当 GITLINK 帮你生成 PPT、图片等文件时，它们会自动保存在这里。';
    }
    return (
...
14. 游戏/动画必须真正可运行——这是最高优先级：
  a. 含 canvas 的游戏：必须有完整的 gameLoop（requestAnimationFrame），循环内先 clearRect 清屏、再遍历所有活跃元素并逐个 draw、最后递归调用自身；禁止只写空循环或只画背景。
  b. 方块/俄罗斯方块/消消乐等游戏：初始化时必须创建第一个活跃方块/元素并加入数组，gameLoop 中必须处理下落/移动/碰撞/生成新方块的逻辑；点击"开始游戏"后 gameLoop 须立即启动。
  c. 在输出前请自行检查：是否真的有代码在 canvas 上绘制了有颜色的矩形/圆/图形？是否有代码在间隔/帧回调中更新坐标？如果答案为否，说明游戏不会运行——必须补全。
  d. 触控适配：移动端无键盘，游戏必须同时支持触摸操作（touch 事件或屏幕上的方向按钮），不能只依赖键盘事件。
  e. canvas 尺寸必须在 JS 中动态设置（canvas.width = 容器宽度, canvas.height = 容器高度），不能写死为 0 或依赖 CSS 百分比（CSS 只影响显示尺寸，不影响绘图分辨率）。`;
...
12. 当用户反馈游戏"没有方块""不动""看不到东西""没反应"等可玩性问题时，必须逐行审查以下关键链路，缺哪个补哪个：
  a. 游戏元素是否被创建并加入数组/状态（如 blocks.push(newBlock)）？
  b. gameLoop / requestAnimationFrame 是否已启动并持续递归调用？
  c. gameLoop 内是否先 clearRect 再遍历数组逐个绘制？
  d. 绘制调用是否使用了可见颜色（fillStyle 不能是透明/白底白色）？
  e. canvas.width / canvas.height 是否在 JS 中被正确赋值（不为 0、不仅依赖 CSS）？
  f. 新元素的生成/下落/移动逻辑是否确实在循环中执行？
  g. 移动端是否有触摸操作方式（不能只靠键盘）？
  修复时不要只加注释说"已修复"——必须写出实际能运行的代码。`;
...
function formatVibekidsUserProfileBlock(user: {
  ...
  if (user.darwinOnboarding != null) {
    try {
      ...
      if (s.length > 2) {
        parts.push(`【GITLINK 问卷/画像（JSON 摘要，供偏好推断）】\n${s}`);
      }
    } catch {
      /* ignore */
    }
...
export function lobsterRoutes(): Router {
  ...
  router.post('/apply', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      console.log(`[Lobster] User ${userId} applied (name="${instance.name ?? 'GITLINK'}"). Total: ${getAllInstances().length}`);
  ...
  router.get('/history', authenticateToken, async (req: AuthRequest, res: Response) => {
    const userId = req.user!.id;
    const instance = getLobsterInstance(userId);
    if (!instance) return res.status(403).json({ error: '请先申请 GITLINK' });
    const conv = getLobsterConversation(userId);
  ...
  router.post('/chat', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    if (!llm) {
      return res.status(402).json({
        error: 'NO_KEY',
        message: 'GITLINK 需要 API Key 才能运行。请使用积分兑换平台虚拟 Key，或在设置中填入自己的 Key。',
      });
    }
    ...
    // 用户给 GITLINK 起的名字（如有）
    const lobsterName = instance.name?.trim() || 'GITLINK';
    let systemContent = lobsterName !== 'GITLINK'
      ? LOBSTER_SYSTEM_PROMPT.replace(/你是"GITLINK"/, `你是"${lobsterName}"（昵称，本质上你是 GITLINK）`)
    ...
    try {
      ...
      for (let step = 0; step < MAX_REACT_STEPS; step++) {
        ...
        if (!toolCalls || toolCalls.length === 0) {
          finalText = choice.message.content?.trim() || '（GITLINK 没有生成回复，请重试）';
          break;
        }
        ...
        if (step === MAX_REACT_STEPS - 1) {
          ...
          if (!finalText) {
            ...
            finalText = fallbackResp.choices[0]?.message?.content?.trim() || '（GITLINK 处理超时，请重新提问）';
          }
        }
      }
  ...
  router.post('/vibekids-generate', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    if (!instance) {
      return res.status(403).json({
        ...
        detail: '请先申请 GITLINK 后再使用 AI 生成；也可退出登录后使用演示模式。',
      });
    }
    ...
    if (!llm) {
      return res.status(402).json({
        error: 'NO_KEY',
        message:
          'GITLINK 需要平台虚拟 Key 才能调用模型。请先在积分兑换中申请 Key，或与 /my-lobster 对话使用相同配置。',
      });
    }
...
请输出修改后的完整 HTML 文件源码。`;
  ...
  router.post('/vibekids-chips', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    if (!instance) {
      return res.status(403).json({
        error: 'darwin_required',
        detail: '请先接入 GITLINK 后再使用个性化灵感提示。',
      });
    }
...
export async function runDarwinJobA2AReply(
  ...
  const instance = getLobsterInstance(userId);
  if (!instance) return { ok: false, error: '请先申请 GITLINK' };

  const llm = await getLlmClient(resolveModel(), userId);
  if (!llm) {
    return {
      ok: false,
      error: 'GITLINK 需要平台虚拟 Key 才能运行，请先在积分兑换中申请 Key。',
    };
  }
  ...
  if (!finalText) {
    finalText = '（GITLINK 暂时无法生成回复，请稍后重试或检查模型配置）';
  }
