...
function usernameFromLinkedInProfile(info: LinkedInUserInfo): string {
  const raw = (
    info.name ||
    [info.given_name, info.family_name].filter(Boolean).join('_') ||
    ''
  ).trim();
  const slug = raw
    .normalize('NFKD')
    ...
    .toLowerCase()
    .slice(0, 28);
  if (slug.length >= 2 && USERNAME_RE.test(slug)) return slug;
  const idPart = info.sub.replace(/\W/g, '').slice(-12) || 'user';
  return `li_${idPart}`.slice(0, 32);
}
...
router.post('/linkedin', async (req: Request, res: Response) => {
  try {
    ...
    if (!code || typeof code !== 'string') {
      return res.status(400).json({ error: 'LinkedIn authorization code is required' });
    }
    if (!redirectUri || typeof redirectUri !== 'string') {
      return res.status(400).json({ error: 'redirectUri is required and must match the OAuth redirect URL' });
    }
    if (!LINKEDIN_CLIENT_ID || !LINKEDIN_CLIENT_SECRET) {
      return res.status(500).json({ error: 'LinkedIn OAuth is not configured on the server' });
    }
    ...
    if (!userinfoRes.ok) {
      return res.status(401).json({ error: 'LINKEDIN_AUTH_FAILED', detail: 'Failed to fetch LinkedIn profile' });
    }
    const info = (await userinfoRes.json()) as LinkedInUserInfo;
    if (!info.sub) {
      return res.status(401).json({ error: 'LINKEDIN_AUTH_FAILED', detail: 'Invalid userinfo response' });
    }

    const linkedInId = info.sub;
    const email =
      info.email && (info.email_verified === undefined || info.email_verified === true)
    ...
    if (!user && email) {
      ...
      if (existingByEmail) {
        if (existingByEmail.linkedInId && existingByEmail.linkedInId !== linkedInId) {
          return res.status(409).json({
            error: 'EMAIL_LINKED_OTHER_LINKEDIN',
            detail: 'This email is already linked to another LinkedIn account',
          });
        }
        user = await prisma.user.update({
    ...
    if (!user) {
      let username = usernameFromLinkedInProfile(info);
      const idClean = linkedInId.replace(/\W/g, '');
      for (let i = 0; i < 8; i++) {
        const taken = await prisma.user.findUnique({ where: { username } });
        if (!taken) break;
        username = `li_${idClean.slice(-10)}${i > 0 ? `_${i}` : ''}`.slice(0, 32);
      }
      if (await prisma.user.findUnique({ where: { username } })) {
        username = `li_${linkedInId.slice(0, 8)}_${Date.now().toString(36)}`.slice(0, 32);
      }
